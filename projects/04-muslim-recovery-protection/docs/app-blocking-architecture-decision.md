...
> **Decision:** APPROVED product direction and bounded architecture, 2026-09-26, by the Owner in
> the project conversation. **Implementation and Play release are conditional on the gates below.**
...
| AB6 | Permission design is proportional and transparent. Benchmark Usage Access plus a block-screen overlay against narrowly configured Accessibility events; choose the least sensitive variant that meets the approved interruption criterion. Any Accessibility declaration requires an in-app prominent disclosure and affirmative consent, accurate Play listing/declaration, and Play approval before release. |
...
condition (new architecture review plus explicit human approval) is met for this bounded design,
not for arbitrary Accessibility use.
...
leave the target**. The spike compares a UsageStatsManager-based signal plus permitted overlay
against `AccessibilityService` window-change events, a narrow service configuration and an
...
|---|---|---|
| Usage Access | Observe app usage / foreground events | Prototype candidate; polling or delayed events may miss the prompt-interruption target. Do not keep a usage timeline. |
...
For a user-facing claim, name the tested apps, Android/OEM version, launch paths and remaining
first-frame exposure. Never say “cannot open”, “unbreakable”, “prevents uninstall” or that DNS
...
The architecture direction above is approved; the enforcement mechanism and strength claim are
**UNKNOWN** until a bounded, harmless-app spike. Pre-register target Android/OEM versions,
selected benign apps, launch paths and the numerical maximum latency **before** measuring.

| Gate | Evidence required | If it fails |
|---|---|---|
| Mechanism | Compare Usage Access/overlay and narrow Accessibility on the same physical device for launcher, notification, deep link, Recents, cold/warm start, split screen, reboot, rapid switches and background restrictions. Capture timing and whether the target can be read or tapped before interruption. | Select the variant supported by results; if neither meets the pre-registered boundary, narrow the claim or reject app blocking. Do not present “immediate” as proven. |
